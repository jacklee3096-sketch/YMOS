...
YMOS 的 SOP 天然兼容 Agent 的外挂能力。当 Agent 有搜索能力，P1 建档自动补足公开信息；当 Agent 挂载了财经数据工具（MX_FinSearch、MX_FinData 等），选股筛选和财务快照自动增强；当同一目录里有其他知识模块（BrainStorm、研究笔记），它们自动成为分析上下文。能力越强，SOP 的执行质量越高——但即使什么都没有，纯 RSS + Yahoo Finance 也能完整跑通。
...
- **首选（本地方案）**：[Hermes](https://hermes-agent.nousresearch.com/) —— Nous Research 出品的本地 AI Agent，支持文件读写、代码执行、定时任务，开源可控。
