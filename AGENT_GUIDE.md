...
Eyes/SOP_市场洞察
  ...
  → scripts: Eyes/scripts/fetch_rss_byMiniflux.py, fetch_market_api.py, fetch_finnhub_news.py
  → writes: Eyes/市场洞察/
...
5. **数据获取优先级**：优先使用金融类 Skill（如 mx-finance-data、mx-finance-search、mx-macro-data、tushare-finance、comparable-company-analysis）获取数据；金融类 Skill 获取不到时，再使用 WebSearch 兜底
...
### 触发条件
当用户输入中包含以下关键词时，强制执行完整SOP流程，**不因文件已存在而跳过任何步骤**：

| 关键词 | 示例 |
|:---|:---|
| `重新` | 重新调研、重新执行、重新分析 |
| `覆盖` | 全量覆盖、覆盖执行 |
| `重新执行` | 重新执行SOP、重新执行策略分析 |

### 执行逻辑
- **跳过存在检查**：即使目标文件已存在，也强制执行完整流程
- **重新获取数据**：如有搜索/Skill能力，重新查询最新数据（不使用缓存）
- **覆盖写入**：最终产出物按原有规则覆盖写入（如同日报告覆盖）
...
**强制执行规则**：所有 SOP 中要求 AI 自主完成搜索的数据，必须全部获取到才能进行分析。

**具体要求**：
1. **完整获取**：按 SOP 步骤要求，使用金融类 Skill（mx-finance-data、mx-finance-search、comparable-company-analysis、mx-macro-data、tushare-finance）优先获取数据；Skill 获取失败时，使用 WebSearch 兜底
2. **数据完整性检查**：获取完成后，检查是否满足分析所需的最小数据集；不满足时标记为「待人工补充」并在报告中明确说明
3. **分析时机**：只有在数据完整（或已标记缺失项）后，才能进行下一步分析；不得在数据不完整的情况下强行生成分析结论
4. **透明度**：在最终报告中注明数据来源、获取状态及缺失项，确保用户了解分析的数据源完整性

### 初始调研强制检查清单

执行 `调研一下 XX` 时，必须完成以下检查：

| 检查项 | 说明 | 数据来源 |
|:---|:---|:---|
| ✅ 财务数据 | 获取最新财报、估值指标 | mx-finance-data |
| ✅ 公告资讯 | 获取最新公告、新闻 | mx-finance-search |
| ✅ 可比公司分析 | 获取竞品财务对比数据 | comparable-company-analysis + WebSearch|
| ✅ 市场动态 | 补充行业趋势、政策变化 | WebSearch（兜底） |
| ✅ 公司研报数据 | 研报数据 | @全自动研报分析师 智能体|

**执行逻辑**：
1. 按优先级顺序调用金融类 Skill
2. 单个 Skill 调用失败不阻断流程，继续尝试下一个
3. 所有金融类 Skill 都失败时，使用 WebSearch 兜底
4. 执行完成后，检查清单中的每项是否已完成，未完成项标记为「待人工补充」
